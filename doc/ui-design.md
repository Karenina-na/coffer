...
- 使用自定义 `_FloatingNavBar`：`Stack + Positioned` 悬浮胶囊底栏，胶囊内部通过 `ClipRRect + BackdropFilter` 提供液态玻璃模糊，胶囊外部仍是实际可交互内容。Shell 不对 `child` 做整条底部截断；主 Tab 的滚动区和 FAB 通过各页面局部 bottom inset 避让胶囊导航。
