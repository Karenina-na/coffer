...
| 2 | `/holdings` | account_balance_wallet_outlined | 资金 | 账户 / 资产 / 转账三合一 |
...
├── /dashboard             // 仪表盘
├── /holdings              // 资金（内含 3 个二级 Tab，支持 ?tab=0|1|2 预选）
├── /events                // 事件
├── /rates                 // 汇率
└── /cards                 // 卡片
...
- **净资产卡片**：显示 `baseCurrency` 下的总额，支持点击切换基准货币（AppBar 货币选择器）。
...
定位：账户 / 资产 / 转账的统一工作台，使用 `TabBar` 承载三个二级 Tab。
...
- `TransferSimulatePage` + `TransferSimulateBody`
...
- **可变偏好状态**（如仪表盘基准货币） → `NotifierProvider<Notifier, T>`，禁止使用已弃用的 `StateProvider`。
- **跨 Feature 聚合**（如仪表盘汇总） → 在 Presentation 层组合多个 Provider + UseCase，Domain 层保持纯粹。

## 7. 主题与 Material 3

- `ColorScheme.fromSeed` 统一取色。
- 使用 `NavigationBar` 而非 `BottomNavigationBar`。
...
- **v4**：`/holdings?tab=` 深度链接、跨实体相互点击跳转、全局搜索 Speed Dial、多跳路径规划（Dijkstra）。
- **v4**：
...
- 拓扑图：`graphview`（力导向 / 树形布局）+ `CustomPainter` 叠加全球地图底图
- 动画：Material 3 `AnimatedSwitcher` / `Hero` / `ImplicitlyAnimatedWidget` 系列
- 数据层：所有聚合指标通过新增 `Provider` 组合已有 Repository 数据，**不新增 Drift 表**；历史趋势依赖已有 `ExchangeRate.asOfTime` + `Asset.valuationTime` + `DomainEvent.triggerTime` 时间序列
...
| K线/折线图 | 单资产价格走势 | `asset_detail_page` | `LineChart` (已有 `_PriceChart` 升级) | `ExchangeRate` 历史 + Yahoo 行情 |
...
组件：`HealthRadarChart`，放置于仪表盘 KPI 网格下方（可折叠面板）。
...
#### 10.3.3 资金页 — 新增「分析」Tab

在现有 `/holdings` 的三个 Tab（账户 / 资产 / 转账）基础上新增第四个 Tab：
...
| `GwpTopologyGraph` | 通道拓扑图（力导向） | `accounts`, `channels`, `accountChannels`, `onNodeSelect`, `highlightPath` |
| `GwpKpiTile` | KPI 指标格 | `title`, `value`, `subtitle`, `icon`, `onTap`, `trend` |
| `GwpProgressRing` | 进度环（信用额度等） | `value: double`, `max: double`, `label`, `color` |
| `GwpCollapsibleSection` | 可折叠分区 | `title`, `count`, `initiallyExpanded`, `child` |
...
**Phase 3 — 通道拓扑图 + 分析 Tab**
- `GwpTopologyGraph` 力导向布局组件
- 转账页拓扑视图 + 路径规划联动
- 资金页「分析」Tab（Top10 / 堆叠柱状 / 热力矩阵）
- 健康雷达图
