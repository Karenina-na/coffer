...
> 项目目录、Dart 包名与用户可见应用名统一为 **Coffer**。
...
- Flutter 3.44.2 + Material 3 + Dart SDK `^3.11.5`
...
│   ├── ui/                      # 共享 UI（EntitySearchDelegate、图表、状态组件等纯组件）
...
└── features/
    ...
    ├── wealth/                  # 跨 Dashboard / Holdings / Account 的净值摘要与趋势 provider
    ├── topology/                # 全景关系图
    ├── backup/                  # 备份 / 恢复
    ├── settings/                # App 级全局设置
    ├── search/                  # 全局搜索编排、命令面板、搜索历史
    └── auth/                    # 生物识别 / 应用锁
```
...
- 所有顶层页面 AppBar 右上 🔍 按钮，基于 `features/search/presentation/GlobalSearchDelegate` 编排；通用列表搜索组件仍复用 `core/ui/entity_search_delegate.dart`
- 每页不同的匹配字段：
  - 账户：机构 / 账号 / 地区 / 类型
  - 资产：代码 / 类型 / 币种
  - 卡片：掩码 / 发卡行 / 组织 / 类型 / 币种
...
```bash
# 若本机使用 FVM 管理 Flutter，优先使用 fvm 前缀：
fvm flutter pub get
fvm dart run build_runner build --delete-conflicting-outputs
fvm flutter run

# 若未使用 FVM，也可直接使用全局 Flutter：
# flutter pub get
...
- **Flutter**：3.44.2 • stable（通过 FVM 固定）；Dart SDK 约束见 `pubspec.yaml`
- **Android SDK**：`~/Library/Android/sdk`（`platform-tools/adb`、`emulator/emulator`）
- **Android 模拟器 AVD**：`coffer_avd`
...
### macOS Flutter / FVM 配置

本项目已通过 `.fvmrc` 固定 Flutter `3.44.2`。Dart SDK 约束为 `^3.11.5`，以 `pubspec.yaml` 为准。如果本机已安装 FVM，建议始终通过项目内版本运行命令，避免全局 Flutter 升级后引入不可预期差异：

```bash
cd /Users/weizixiang/Workspace/Project/coffer

# 首次：安装并绑定项目 Flutter 版本。
fvm install 3.44.2
fvm use 3.44.2

# 自检：确认 Flutter / Dart / Android toolchain。
fvm flutter --version
fvm flutter doctor -v
```

后续开发命令统一使用 `fvm flutter ...` / `fvm dart ...`。若希望在当前 shell 中直接输入 `flutter`，可先设置 FVM 全局版本，再把 FVM default SDK 加到 PATH；项目文档与脚本示例仍优先写 FVM 前缀：
...
- VS Code：把 Dart SDK 指向 `.fvm/flutter_sdk/bin/cache/dart-sdk`，Flutter SDK 指向 `.fvm/flutter_sdk`。
- Android Studio / IntelliJ：Flutter SDK path 选择项目内 `.fvm/flutter_sdk`。
- `.fvmrc` 已提交为项目版本锁；`.fvm/flutter_sdk` 等本地 SDK 链接/缓存不提交。
...
fvm flutter test                                       # 全部 pass
fvm flutter build apk --debug                          # 产出 build/app/outputs/flutter-apk/app-debug.apk
fvm flutter build apk --release                        # 产出 build/app/outputs/flutter-apk/app-release.apk
...
```bash
# macOS
~/Library/Android/sdk/emulator/emulator -avd coffer_avd

# Windows WSL2（必须用 x86_64 镜像）
nohup $ANDROID_HOME/emulator/emulator -avd coffer_avd -port 5554 -no-boot-anim &
```

### Android 模拟器 / AVD 配置

当前基线机器使用的 AVD（`~/.android/avd/coffer_avd.avd/config.ini` 摘要）：
...
# 4. 列出确认
avdmanager list avd | grep coffer_avd

# 5. 启动
emulator -avd coffer_avd &
```

> **Intel Mac**：把两个 `arm64-v8a` 改成 `x86_64`。
>
> **希望更大内存 / 存储**：创建后可直接编辑 `~/.android/avd/coffer_avd.avd/config.ini`：
> - `hw.ramSize = 3072`（MB）
...
avdmanager delete avd -n coffer_avd                 # 删除
emulator -list-avds                              # 仅看 AVD 名称
emulator -avd coffer_avd -wipe-data                 # 重置用户分区（解决首启异常）
emulator -avd coffer_avd -no-snapshot-load          # 冷启动，不从快照恢复
emulator -avd coffer_avd -gpu host                  # 强制宿主 GPU
```
...
~/Library/Android/sdk/emulator/emulator -avd coffer_avd -netdelay none -netspeed full &
...
~/Library/Android/sdk/platform-tools/adb shell am start \
  -n com.coffer.coffer/.MainActivity         # 包名以 android/app/build.gradle.kts 为准
...
- **模拟器启动后黑屏 / 卡 launcher** → `emulator -avd coffer_avd -wipe-data` 重置用户分区
...
- **安装失败 `INSTALL_FAILED_UPDATE_INCOMPATIBLE`** → 签名/包名变了，先 `adb uninstall com.coffer.coffer`
- **Drift schema 变更后首启崩溃** → 检查 `onUpgrade` 是否覆盖当前 `from`；开发期可 `adb shell pm clear com.coffer.coffer` 清数据重来
...
nohup $ANDROID_HOME/emulator/emulator -avd coffer_avd -port 5554 \
...
unset ADB_SERVER_SOCKET && ADB_MDNS=0 adb shell am start \
  -n com.coffer.coffer/.MainActivity
...
- **模拟器启动后黑屏**：`emulator -avd coffer_avd -wipe-data` 重置用户分区。
- **安装失败 `INSTALL_FAILED_UPDATE_INCOMPATIBLE`**：签名/包名变了，先 `adb uninstall com.coffer.coffer`。
- **Drift schema 变更后首启崩溃**：检查 `onUpgrade` 是否覆盖当前 `from` 区间；开发期可 `adb shell pm clear com.coffer.coffer` 清数据重来。
